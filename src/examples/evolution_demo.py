...
def worker(func, x):
    population.append(np.copy(x))
    return func(x.T)

# Define the optimisation function using Differential Evolution
def evolutionary_optimisation(func, bounds, strategy='best1bin', maxiter=1000, popsize=3):
    optimiser = differential_evolution(func, bounds, strategy=strategy, maxiter=maxiter, popsize=popsize, tol=1e-7, seed=None, disp=True, workers=worker)
...
if __name__ == "__main__":
    ...
    print(f"Optimised X corresponds: {func(optimised_X)}")
